                ...
                <form action="{{route('portfolio.update', $portfolio->id)}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title" id="title" class="form-control"
                            placeholder="Ex. Have a Good News" value="{{$portfolio->title}}">
                    </div>
                    <div class="form-group row">
                        <div class="col-6">
                            <label for="slug">Slug</label>
                            <input type="text" name="slug" id="slug" class="form-control"
                                placeholder="Ex. have-a-good-portfolio" value="{{$portfolio->slug}}">
                        </div>
                        <div class="col-6">
                            <label for="category">Category</label>
                            <select class="form-control" id="category" multiple="multiple" name="category[]">
                                @foreach ($category as $item)
                                <option value="{{$item->id}}" @foreach ($portfolio->category as $c)
                                    @if ($item->id == $c->id)
                                    selected="selected"
                                    @endif
                                    @endforeach
                    ...
                    <div class="checkbox">
                        <label class="ui-check">
                            <input type="checkbox" value="0" name="draft" @if ($portfolio->isDraft == 0)
                            checked
                            @endif>
